...
export function FacebookPreview({ pageName, imageBase64, caption, hashtags }: PlatformPreviewProps) {
  return (
    <div className="overflow-hidden rounded-lg bg-white text-[#000000e6]">
      <div className="flex items-start gap-2 p-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#1877f2] text-[13px] font-bold text-white">
      ...
      <p className="whitespace-pre-wrap px-3 pb-2 text-[13px] leading-snug">
        {caption}
        {hashtags.length > 0 && <span className="text-[#1877f2]"> {hashtags.join(" ")}</span>}
      </p>
      {imageBase64 && (
